...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    gain_points = 0
    if num_rolls == 0 and opponent_score == 0:
        gain_points = 1
    elif num_rolls == 0 and opponent_score != 0:
        gain_points = hefty_hogs(player_score=player_score, opponent_score=opponent_score)
    else:
        gain_points = roll_dice(num_rolls=num_rolls, dice=dice)
    return gain_points
    # END PROBLEM 3
...
def hog_pile(player_score, opponent_score):
    ...
    if player_score % 10 == opponent_score % 10:
        additional_points = player_score % 10
    else:
        additional_points = 0
